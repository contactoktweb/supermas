...
async function runCustomerTests() {
  ...
  const posByDoc = await customerService.searchForPos(testDocNumber)
  assert(posByDoc.length > 0 && posByDoc[0].documentNumber === testDocNumber, 'Búsqueda POS por documento exitosa')
